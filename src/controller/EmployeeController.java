...
public class EmployeeController {
    ...
    public static List<EmployeeModel> getAllEmployee() {
        ...
        String query = "SELECT e.employee_id, a.username ,concat(e.first_name, ' ', e.last_name) as full_name, e.email, e.phone, e.address, e.position, e.department from employees as e left join accounts as a on e.account = a.account_id";
    ...
    public boolean addEmployee(EmployeeModel employee) {
        ...
        try (Connection con = DatabaseConnection.getConnection();
            ...
            acc_pstmt.setString(1, employee.getAccount() + "_" + employee.getEmployee_id());
            ...
            String employee_query = "INSERT INTO employees (employee_id, account, first_name, last_name, email, phone, address, position, department, hire_date, salary)\n" +
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?)";
            try (PreparedStatement emp_pstmt = con.prepareStatement(employee_query)) {
                emp_pstmt.setString(1, employee.getEmployee_id());
                emp_pstmt.setInt(2, account_id);
                emp_pstmt.setString(3, employee.getEmployee_first_name());
                emp_pstmt.setString(4, employee.getEmployee_last_name());
                emp_pstmt.setString(5, employee.getEmployee_email());
                emp_pstmt.setString(6, employee.getEmployee_phone());
                emp_pstmt.setString(7, employee.getEmployee_address());
                emp_pstmt.setString(8, employee.getEmployee_position());
                emp_pstmt.setString(9, employee.getEmployee_department());
                emp_pstmt.setDate(10, employee.getEmployee_date());
                emp_pstmt.setString(11, employee.getEmployee_salary());
    ...
    public EmployeeModel getEmployee(String employeeID) {
        ...
        String query = ""
                ...
                + "FROM employees AS e "
                + "LEFT JOIN accounts AS a ON e.account = a.account_id "
                + "WHERE e.employee_id = ?";

        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ptsmt = con.prepareStatement(query);
        ) {
            ptsmt.setString(1, employeeID);
    ...
    public boolean updateEmployee(EmployeeModel employee) throws SQLException {
        ...
        if(isExistEmployee(employee.getEmployee_email()) == true){
            return false;
        }
        try (Connection con = DatabaseConnection.getConnection()) {
            isSeccess = false;
            String isExistAccountID = "SELECT account from employees where employee_id = ?";
            PreparedStatement check_acc = con.prepareStatement(isExistAccountID);
            check_acc.setString(1, employee.getEmployee_id());
            ...
            if (rs.next() == false) {
                ...
                String insertIDIntoEmployee = "UPDATE employees SET account=? WHERE employee_id=? VALUES (?,?)";
                PreparedStatement emp_pstmt = con.prepareStatement(insertIDIntoEmployee);
                emp_pstmt.setInt(1, account_id);
                emp_pstmt.setString(2, employee.getEmployee_id());

                String updateQuery = "UPDATE employees AS e" +
                        "LEFT JOIN accounts AS a ON e.account = a.account_id " +
                        "SET e.account = a.account_id," +
                        "e.first_name = ?," +
                        "    e.last_name = ?," +
                        "    e.email = ?," +
                        "    e.phone = ?," +
                ...
                update.setString(13, employee.getEmployee_id());
    ...
    public boolean isExistEmployee(String employeeEmail) throws SQLException {
        boolean isExist = false;
        String query = "SELECT distinct(email) FROM employees WHERE email = ?";
        try(Connection con = DatabaseConnection.getConnection();
    ...
    public boolean deleteEmployee(String employeeId) throws SQLException {
        ...
        String updateIssueQuery = "UPDATE issues i\n" +
                "JOIN employees e ON i.employee_id = e.employee_id\n" +
                "SET i.employee_name = concat(e.last_name ,' ',e.first_name)\n" +
                "WHERE e.employee_id = ?;\n";
        try(Connection conn = DatabaseConnection.getConnection();
        PreparedStatement pstmtt = conn.prepareStatement(updateIssueQuery)) {
            pstmtt.setString(1, employeeId);
